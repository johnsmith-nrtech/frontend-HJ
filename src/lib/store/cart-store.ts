...
//
//  Backend stores:
//    variant.price             = BASE / original price
//    variant.compare_price     = "was" price (higher), shown struck-through
//    variant.discount_percentage = direct % off the base price
//
//  Sale price calculation (identical to product-details.tsx):
//
//  Priority 1 — compare_price:
//    discountPct = round((compare_price - price) / compare_price * 100)
//    salePrice   = price - (price * discountPct / 100)
//
//  Priority 2 — discount_percentage:
//    salePrice   = price * (1 - discount_percentage / 100)
//
//  Priority 3 — no discount:
//    salePrice   = price
// ─────────────────────────────────────────────────────────────────
const resolveDisplayPrice = (variant: ApiCartItem["variant"]): number => {
  ...
  if (variant.discount_percentage && variant.discount_percentage > 0) {
    const salePrice = basePrice * (1 - variant.discount_percentage / 100);
    return Math.round(salePrice * 100) / 100;
  }
