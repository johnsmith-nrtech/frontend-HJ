...
const emptyMaterialInfo = {
  care_instructions: "",
  scatter_cushion_cover: "",
  scatter_cushion_filling: "",
  frame_info: "",
  seat_base_info: "",
  seat_cushion_info: "",
  back_support_info: "",
...
export function VariantManager({
  ...
  const addVariant = async () => {
    ...
    try {
      if (editingIndex !== null) {
        ...
        // Bed options only apply to bed variants; never persist them on a sofa
        updateData.bed_options = isBed ? (newVariant.bed_options || emptyBedOptions) : undefined;
        ...
        if (existingImages.length > 0) {
          ...
          const updatePromises = existingImages.map(async (img, index) => {
            ...
            if (originalOrder !== index) {
              try {
                return await updateImageDetailsMutation.mutateAsync({
                  imageId,
                  data: { order, type: imageType },
                });
              } catch (error) {
                console.error(`Failed to update image ${imageId.slice(-8)}:`, error);
                throw error;
              }
            }
          });
        ...
        if (newImages.length > 0) {
          ...
          const imageType = existingImages.length > 0 ? "gallery" : "main";
        ...
        const createdVariant = await createVariantMutation.mutateAsync(createData);
  ...
  setNewVariant({
    ...
    bed_options: variant.bed_options || emptyBedOptions,
  });
              ...
              <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
                ...
                <div>
                  ...
                  />
                            ...
                            {/* Material Composition & Construction Section — sofa only */}
              {!isBed && (
              <div className="space-y-3 border-t pt-4">
                ...
                <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
                  <div className="col-span-2">
                    ...
                    <Input
                      placeholder="e.g., Upholstered In Fabric With A Soft, Textured Touch..."
                      value={newVariant.material_info?.care_instructions || ""}
                      onChange={(e) =>
                        setNewVariant({
                          ...newVariant,
                          material_info: {
                            ...newVariant.material_info,
                  ...
                  <div>
                    <label className="text-sm font-medium">Scatter Cushion Cover</label>
                    <Input
                      placeholder="e.g., 100% Polyester"
                      value={newVariant.material_info?.scatter_cushion_cover || ""}
                      onChange={(e) =>
                        setNewVariant({
                          ...newVariant,
                          material_info: {
                            ...newVariant.material_info,
                            scatter_cushion_cover: e.target.value,
                          },
                        })
                      }
                      disabled={disabled}
                  ...
                  <div>
                    <label className="text-sm font-medium">Scatter Cushion Filling</label>
                    <Input
                      placeholder="e.g., Fibre-Filled Cushions"
                      value={newVariant.material_info?.scatter_cushion_filling || ""}
                      onChange={(e) =>
                        setNewVariant({
                          ...newVariant,
                          material_info: {
                            ...newVariant.material_info,
                            scatter_cushion_filling: e.target.value,
                          },
                        })
                      }
                      disabled={disabled}
                  ...
                  <div>
                    ...
                    <Input
                      ...
                      value={newVariant.material_info?.frame_info || ""}
                      onChange={(e) =>
                        setNewVariant({
                          ...newVariant,
                          material_info: {
                            ...newVariant.material_info,
                            frame_info: e.target.value,
                          },
                        })
                      }
                      disabled={disabled}
                  ...
                  <div>
                    <label className="text-sm font-medium">Seat Base</label>
                    <Input
                      placeholder="e.g., Serpentine Springs"
                      value={newVariant.material_info?.seat_base_info || ""}
                      onChange={(e) =>
                        setNewVariant({
                          ...newVariant,
                          material_info: {
                            ...newVariant.material_info,
                            seat_base_info: e.target.value,
                          },
                        })
                      }
                      disabled={disabled}
                  ...
                  <div>
                    <label className="text-sm font-medium">Seat Cushion</label>
                    <Input
                      placeholder="e.g., Foam-Filled, Fibre-Topped"
                      value={newVariant.material_info?.seat_cushion_info || ""}
                      onChange={(e) =>
                        setNewVariant({
                          ...newVariant,
                          material_info: {
                            ...newVariant.material_info,
                            seat_cushion_info: e.target.value,
                          },
                        })
                      }
                      disabled={disabled}
                  ...
                  <div>
                    <label className="text-sm font-medium">Back Support</label>
                    <Input
                      placeholder="e.g., Tensioned Webbing"
                      value={newVariant.material_info?.back_support_info || ""}
                      onChange={(e) =>
                        setNewVariant({
                          ...newVariant,
                          material_info: {
                            ...newVariant.material_info,
                            back_support_info: e.target.value,
                          },
                        })
                      }
                      disabled={disabled}
                  ...
                  <div>
                    <label className="text-sm font-medium">Back Cushion</label>
                    <Input
                      placeholder="e.g., Fibre-Filled"
                      value={newVariant.material_info?.back_cushion_info || ""}
                      onChange={(e) =>
                        setNewVariant({
                          ...newVariant,
                          material_info: {
                            ...newVariant.material_info,
                            back_cushion_info: e.target.value,
                          },
                        })
                      }
                      disabled={disabled}
                    ...
                    <div className="flex gap-4">
                        ...
                        {variant.images && variant.images.length > 0 ? (
                          <div className="relative h-16 w-16 overflow-hidden rounded-md border bg-white">
                            <Image
                              fill
                              src={
                                variant.images.find((img) => img.type === "main")?.url ||
                                variant.images.find((img) => img.type === "main")?.preview ||
                      ...
                      <div className="flex-1 space-y-2">
                        ...
                        <div className="grid grid-cols-2 gap-4 text-sm md:grid-cols-4">
                          <div>
                            ...
                            {variant.compare_price && variant.compare_price > variant.price && (
                              <Badge variant="destructive" className="ml-2">
                                {calculateDiscount(variant.price, variant.compare_price)}% OFF
                              </Badge>
                            )}
                          </div>
                          <div>
                          ...
                          {variant.weight_kg && (
                            <div>
                              <span className="font-medium">Weight:</span> {variant.weight_kg}kg
                            </div>
                          )}
                          {variant.delivery_time_days && (
                            <div>
                              <span className="font-medium">Delivery:</span> {variant.delivery_time_days}
                            </div>
                          )}
                          {variant.assemble_charges && (
                            <div>
                              <span className="font-medium">Assemble Charges:</span> £{variant.assemble_charges}
                            </div>
                          )}
                          {variant.material && (
                            <div>
                              <span className="font-medium">Material:</span> {variant.material}
                            </div>
                          )}
                          {variant.brand && (
                            <div>
                              <span className="font-medium">Brand:</span> {variant.brand}
                            </div>
                          )}
                          {variant.tags && (
                            <div className="col-span-2">
