  ...
  useEffect(() => {
    if (isSearchMode && isInitialized && filteredResults.length > 0) {
      let transformed = filteredResults.map((result): PageProduct => {
        ...
        const imageUrl = mainImage?.url || firstImage?.url || "/placeholder.svg";
    ...
    !isSearchMode && apiProducts?.items
      ? apiProducts.items.map((product) => {
          ...
          if (product.variants && product.variants.length > 0) {
            ...
            selectedVariant =
              sizeMatch || materialMatch || featured || inStock || product.variants[0];
          }
          ...
          const imageUrl =
            variantImage?.url || mainImage?.url || firstImage?.url || "/placeholder.svg";
